...
open class BlueTextureBehaviour: BaseBehaviour {
    ...
    open override func onCreate() -> Void {
        ...
        self.renderAsset = GLES2TextureAsset(GLES2TextureAssetPath: "Textures/texture02.png")
        self.renderAsset.create()
        self.renderAsset.transform.position = GLKVector3(v: (2.0, 0, 0))
        self.collider = SphereCollider(renderAsset: self.renderAsset)
    ...
    open override func onUpdate(delta: TimeInterval) -> Void {
        let npos: GLKVector3 = Bezier.create(currentTime: self.time, totalTime: 5.0, start: GLKVector3(v: (2.0, 0, -5.0)), control: GLKVector3(v: (1, 5, -2.5)), end: GLKVector3(v: (0.0, 0, 0)))
